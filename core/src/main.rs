...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let peripheral_futures = peripherals.into_iter().map(|p| async move {
        p.properties().await.ok().flatten().map(|props| (p, props))
    });
    let found_peripherals = join_all(peripheral_futures).await;
    let dialog_peripheral = found_peripherals.into_iter().filter_map(|x| x).find(|(_, props)| {
        props.local_name.as_ref().map_or(false, |name| name.contains("Dialog Peripheral"))
    }).map(|(p, _)| p).ok_or("Device not found")?;
    ...
    if let Some(service) = services.iter().find(|&s| s.uuid == data_service_uuid) {
        if let Some(characteristic) = service.characteristics.iter().find(|&c| c.uuid == data_characteristic_uuid) {
    ...
    while let Some(notification) = notification_stream.next().await {
        match notification {
            ValueNotification { uuid, value, .. } if uuid == data_characteristic_uuid => {
                println!("Received notification on {:?}: {:?}", uuid, value.iter().map(|byte| format!("{:02X}", byte)).collect::<Vec<String>>().join(" "));
            }
            _ => {}
        }
    }
